(function($) { 
	...
	function calendarWidget(el, params) { 
		...
		var table = '';
			...
			table += ('<h3 id="current-month"> <i class="icon-chevron-left" style="margin:5px 20px 0px -70px" onClick="calLeft();"></i>'+monthNames[month]+' '+year+'<i class="icon-chevron-right" style="margin:5px 60px 0px 20px" onClick="calRight();"></i></h3>');
            ...
            for (j=0;j<42;j++){
              ...
              if ((j<firstDay)){
                table += ('<td class="other-month" style ="background-color: #D8D8D8"><span class="day">'+ (prev_days-firstDay+j+1) +'</span></td>');
			  } else if ((j>=firstDay+getDaysInMonth(month,year))) {
				i = i+1;
                table += ('<td class="other-month" style ="background-color: #D8D8D8"><span class="day">'+ i +'</span></td>');			 
              }else{
	...
	$.fn.setDay = function(day, busy) {	
		if (busy) {
			$(".day" + day).addClass("booked");	
	 	}
	 	else {
			$(".day" + day).addClass("notBooked");
	 	}
	}
